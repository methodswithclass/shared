...
(function (obj) {
  ...
  var promises = {};
  var index = {};
  var numEvents = 0; // runs a saved promise

  var future = function future(name) {
    try {
  ...
  var defer = function defer(name, _config) {
  ...
  var dispatch = function dispatch(name, id) {
    // console.log("dispatch event", name);
    var result = {};
    var sub;

    var runEvent = function runEvent(index) {
      try {
  ...
  var on = function on(name, id, _event) {
...
(function (obj) {
  ...
  var createObs = function createObs(input) {
  ...
  var subscribe = function subscribe(input) {
...
(function (obj) {
  ...
  var checkArray = function checkArray(_item, array) {
    for (i in array) {
      if (_item == array[i]) {
        return true;
      }
  ...
  var back = function back() {
    var self = this; // setup a named key/value object to receive data at a later time
  ...
  var save = function save() {
    var self = this; // add data to an array to be retrieved later
...
(function (obj) {
  ...
  var _mobile = false; // force the following checks to return true, render the mobile site on desktop for debugging purposes

  var forceMobile = function forceMobile() {
    _mobile = true;
  }; // blanket check for any mobile vs desktop user agent


  var checkMobile = function checkMobile(forceMobile) {
    var check = false;
  ...
  var whatDevice = function whatDevice(forceMobile) {
    if (_mobile || forceMobile) return mobile;else if (navigator.appName == 'Microsoft Internet Explorer' || !!(navigator.userAgent.match(/Trident/) || navigator.userAgent.indexOf('Edge') != -1 || navigator.userAgent.match(/rv:11/))) {
      return ie;
    } else if (navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/webOS/i) || navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPod/i) || navigator.userAgent.match(/iPad/i) || navigator.userAgent.match(/Blackberry/i)) {
      return mobile;
  ...
  var isPortrait = function isPortrait() {
    ...
    var height = $(window).height(); //console.log("width " + width + " height " + height);
  ...
  var doesExist = function doesExist(item, type) {
    var $type = _typeof(item).toString().toLowerCase(); // console.log("$type", $type);


    return type ? $type === type : $type !== "undefined";
  };

  var sum = function sum(array, $callback) {
    var sum = 0;

    var callback = function callback(value, index, array) {
      return value;
    };
  ...
  var average = function average(array, $callback) {
    var total = 0;
    ...
    if ($callback) callback = $callback;
    total = sum(array, callback); // console.log("total is", total);

    return total / array.length;
  };

  var valueFunc = function valueFunc(value, index, array) {
    return value;
  };

  var valueParam = function valueParam($value) {
  ...
  var round = function round(number, order) {
    var value = Math.round(number / order) * order;
    return value;
  };

  var resolveDigitString = function resolveDigitString(digit) {
    if (digit < 10) {
      return "0" + digit;
    } else {
      return "" + digit;
  ...
  var leadingzeros = function leadingzeros(number, zeros) {
    if (!zeros) zeros = 1;
  ...
  var sort = function sort(array, which, key) {
    var temp;
  ...
  var linear = function linear(params) {
    var y1 = params.y1;
    var y2 = params.y2;
    var x1 = params.x1;
    var x2 = params.x2;
  ...
  var waitForElem = function waitForElem(options, complete) {
    var c = {
      noexist: "noexist",
      found: "found",
      notfound: "notfound"
    ...
    var checkElements = function checkElements(array) {
    ...
    var waitTimer = setInterval(function () {
      if (checkElements(options.elems) == c.noexist) {
        ...
        } else {// console.log("count limit reached");
        }
      } else {
        count++;
      }
  ...
  var fixInside = function fixInside(params) {
    var i = params.inside;
    var s = params.space;
    var iw = i.width;
    var ih = i.height;
    ...
    var goodAspect = function goodAspect(width, height) {
      if (Math.abs(iw / ih - ar) < 0.01) return true;
      return false;
    };

    var checkHeight = function checkHeight($h) {
      if ($h < sh) return "under";else if ($h > sh * 1.2) return "over";
      return "good";
    };

    var checkWidth = function checkWidth($w) {
      if ($w < sw) return "under";else if ($w > sw * 1.2) return "over";
      return "good";
    };
